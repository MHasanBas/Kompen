...
class _UploadProofScreenState extends State<UploadProofScreen> {
  ...
  Future<Map<String, dynamic>> fetchTaskDetails(String tugasId) async {
    ...
    try {
      final response = await dio.post(
        'http://192.168.194.83:8000/api/show_tugas', // Replace with the correct API URL
  ...
  void uploadFile() async {
    ...
    if (result != null) {
      ...
      try {
        final response = await dio.post(
          'http://192.168.194.83:8000/api/upload', // Replace with your actual API URL
          data: formData,
        );
  ...
  void sendTaskToApi() async {
    ...
    try {
      final response = await dio.post(
        'http://192.168.194.83:8000/api/kirim', // Your API URL for sending the task
        data: {
